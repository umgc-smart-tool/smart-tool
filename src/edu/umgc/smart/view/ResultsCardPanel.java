...
public class ResultsCardPanel extends CardPanel {
    ...
    private void initializePanel() {
        ...
        addSearchPanel(BorderLayout.NORTH);
        addResultsPanel(BorderLayout.CENTER);
    }

    private void addSearchPanel(String location) {
        JPanel searchPanel = new JPanel();
        searchPanel.setLayout(new GridBagLayout());
        initializeSelectButtons();
        addActionListeners();
        addComponentsToSearchPanel(searchPanel);
        this.add(searchPanel, location);
    }
    ...
    private void addActionListeners() {
        ...
        addSelectButtonListenersWithLoop();
    }

    private void addSelectButtonListenersWithLoop() {
        for (int i = 0; i < selectButtons.length; i++) {
            final int finalI = i;
            selectButtons[i]
    ...
    private void addResultsPanel(String location) {
        ...
        resultsPanel.setBorder(
                BorderFactory.createTitledBorder(
                        "Results for: \"" + searchTerm + "\" in field: " + searchTermField));
        addResultsToPanel(resultsPanel);
        this.add(resultScrollPane, location);
    }

    private void addResultsToPanel(JPanel resultsPanel) {
        if (records.length > 0) {
            addResultsToPanelWithLoop(resultsPanel);
        } else {
    ...
    private void addResultsToPanelWithLoop(JPanel resultsPanel) {
        for (int i = 0; i < records.length; i++) {
            JPanel recordPanel = new JPanel();
            recordPanel.setSize(750, 500);
            ...
            recordPanel.add(new JLabel(records[i].toString()));
