...
public class SwingView extends View {
  ...
  private void advancedSearchWindow(Point location) {
    ...
    searchButtons[0].addActionListener(e -> LOGGER.info("Searching by reference number: " + searchFields[0].getText()));

    // Title field search button
    searchButtons[1].addActionListener(e -> LOGGER.info("Searching by title: " + searchFields[1].getText()));

    // Record Type field search button
    searchButtons[2].addActionListener(e -> LOGGER.info("Searching by record type: " + searchFields[2].getText()));

    // Author field search button
    searchButtons[3].addActionListener(e -> LOGGER.info("Searching by author: " + searchFields[3].getText()));

    // Date field search button
    searchButtons[4].addActionListener(e -> LOGGER.info("Searching by date: " + searchFields[4].getText()));

    // Category field search button
    searchButtons[5].addActionListener(e -> LOGGER.info("Searching by category: " + searchFields[5].getText()));

    // Summary field search button
    searchButtons[6].addActionListener(e -> LOGGER.info("Searching by summary: " + searchFields[6].getText()));
   ...
   * search.
   *
   * @param location      - This parameter defines the location of the window on
   *                      the screen.
   * @param viewType      - This parameter helps identify which view of this
   *                      window should be shown.
   * @param currentRecord - This is the record that is to be displayed under the
   *                      'VIEW' view type.
   */
   ...
   * method is the GUI for
   * displaying the results of a search. The results are stored in the records
   * array, and are
   * formatted for output within the GUI. Select buttons allow users to select the
   * records to
   * see them in more detail using the viewRecordWindow() method.
   *
   * @param location   - Location of the window on the screen.
   * @param searchTerm - The term used to search resulting in this window, used
   *                   for telling user the term entered.
   * @param records    - The array of Record objects. These are the actual search
   *                   results, used to create results in GUI.
   */
  ...
  /**
   * Method to create a test array of Record objects. ONLY for testing, can be
   * deleted once the
  ...
  private Record[] makeTestRecords(int numberOfRecords) {
    ...
    for (int i = 0; i < numberOfRecords; i++) {
      testRecords[i] = new Record.Builder("R-22-" + i)
          .title("Title " + i)
          .type("MEMO")
          .lastName("Longo")
          .firstName("Josh")
